...
enum CommandType {
  /**
   * \~English Set provided coordinates as home. Thhis command is expected to be
   * the first in the mission.
   * \note The actual home point may differ from set one.
   * \~Russian Назначение заданной точки домом. Ожидается, что эта команда --
   * первая в миссии.
   * \note Фактическое место дома может не совпадать с указанным в миссии.
   */
  HOME,
  /**
   * \~English Command to start flight by taking specified altitude. Expected to
   * follow \ref HOME command.
   * \~Russian Предписание подняться на заданную высоту. Ожидается, что эта
   * команда следует за командой \ref HOME.
   */
  TAKEOFF,
  /**
   ...
   * \note If specified latitude and longitude are equal zero, the landing will
   * be performed at current drone location.
   * \~Russian Предписание приземлиться в указанной точке.
   * \note Если указанные широта и долгота равны нулю, посадка будет произведена
   * в текущем местонахождении дрона.
   */
  LAND,
  /**
  ...
  SET_SERVO,
  /**
   * \~English Command to wait for specified amount of seconds before next
   * command execution.
   * \~Russian Ожидание указанного числа секунд перед выполнением следующей
   * команды.
   */
  DELAY,
  /**
...
struct CommandTakeoff {
   ...
   */
  CommandTakeoff(int32_t alt) { altitude = alt; }
};

/**
 * \~English Structure to store \ref WAYPOINT command arguments (as well as \ref
 * HOME, \ref LAND and \ref INTEREST).
 * \~Russian Структура для хранения аргументов команды \ref WAYPOINT (а также
 * \ref HOME, \ref LAND и \ref INTEREST).
 */
struct CommandWaypoint {
   ...
   * \note Relative for \ref WAYPOINT and \ref INTEREST commands, absolute --
   * for \ref HOME and \ref LAND.
   * \~Russian Высота в см.
   * \note Относительная для команд \ref WAYPOINT и \ref INTEREST, абсолютная --
   * для \ref HOME и \ref LAND.
   */
  int32_t altitude;
...
struct CommandServo {
   ...
   * \note Нумерация начинается с 1. Первые сервоприводы отностяся к двигателям
   * дрона.
   */
  int32_t number;
  /**
...
struct CommandDelay {
   ...
   */
  CommandDelay(int32_t delay_) { delay = delay_; }
};
...
union CommandContent {
  ...
  /**
   * \~English \ref HOME, \ref WAYPOINT, \ref INTEREST and \ref LAND command
   * arguments. See \ref CommandWaypoint.
   * \~Russian Аргументы команд \ref HOME, \ref WAYPOINT, \ref INTEREST и \ref
   * LAND. См. \ref CommandWaypoint.
   */
...
/**
 * \~English Converts a mission received from the ATM server into an array of
 * commands of \ref MissionCommand type.
 * \param[in] mission Mission from the ATM server. Expected form is
 * "$FlightMission mission#".
 * \return Returns 1 on successful conversion, 0 otherwise.
 * \~Russian Преобразует миссию, полученную от сервера ОРВД, в массив команд
 * типа \ref MissionCommand.
 * \param[in] mission Миссия, пришедшая от сервера ОРВД. Ожидается в виде
 * "$FlightMission миссия#".
 * \return Возвращает 1, если миссия была успешно распознана, иначе -- 0.
 */
int loadMission(char *mission);
/**
 * \~English Logs an existing mission and at the same time prints it to the
 * console.
 * \~Russian Записывает имеющуюся миссию в лог и одновременно выводит ее в
 * консоль.
 */
void printMission();
/**
...
/**
 * \~English Converts an array of commands to a string, that can be recognised
 * by the ATM server.
...
int missionToString(MissionCommand *commands, uint8_t num, char *string,
                    uint32_t len);
/**
 * \~English Converts an array of commands to a byte array, that can be
 * recognised by the autopilot firmware.
 * \param[in] commands Array of commands to convert.
 * \param[in] num Number of commands to convert.
 * \param[out] bytes Byte array, where the converted commands will be stored.
 * \return Returns 1 on successful conversion, 0 otherwise.
 * \~Russian Преобразует массив команд в массив байтов, распознаваемый прошивкой
 * автопилота.
 * \param[in] commands Массив команд, которые будут конвертированы.
 * \param[in] num Количество конвертируемых команд.
 * \param[out] bytes Массив байтов, куда будут записаны конвертированные
 * команды.
 * \return Возвращает 1 при успешном преобразовании, иначе -- 0.
 */
 ...
 * \~Russian Вычисляет размер массива для хранения миссии в виде массива
...
/**
 * \~English Converts a no-flight areas string received from the ATM server into
 * an array of areas of \ref NoFlightArea type.
 * \param[in] areas String from the ATM server. Expected form is
 * "$ForbiddenZones areas#".
 * \return Returns 1 on successful conversion, 0 otherwise.
 * \~Russian Преобразует строку с бесполетным зонами, полученную от сервера
 * ОРВД, в массив зон типа \ref NoFlightArea.
 * \param[in] areas Строка, пришедшая от сервера ОРВД. Ожидается в виде
 * "$ForbiddenZones зоны#".
 * \return Возвращает 1, если зоны были успешно распознаны, иначе -- 0.
 */
int loadNoFlightAreas(char *areas);
/**
 * \~English Converts a string with changes in no-flight areas received from the
 * ATM server, and updates current no-flight areas array.
 * \param[in] areas String from the ATM server. Expected form is
 * "$ForbiddenZonesDelta changes#".
 * \return Returns 1 on successful conversion, 0 otherwise.
 * \~Russian Преобразует строку с изменениями бесполетных зон, полученную от
 * сервера ОРВД, и обновляет массив бесполетных зон.
 * \param[in] areas Строка, пришедшая от сервера ОРВД. Ожидается в виде
 * "$ForbiddenZonesDelta изменения#".
...
/**
 * \~English Logs current no-flight areas and at the same time prints them to
 * the console.
 * \~Russian Записывает имеющиеся бесполетные зоны в лог и одновременно выводит
 * их в консоль.
 */
 ...
 * \param[in] response String from the ATM server. Substring
 * "$ForbiddenZonesHash hash$" is expected.
 ...
 * \param[in] response Строка, пришедшая от сервера ОРВД. Ожидается наличие
 * подстроки "$ForbiddenZonesHash хэш$".
...
/**
 * \~English Extracts hash from the string received from the ATM server.
 * \param[in] response String from the ATM server. Substring "$Delay delay#" is
 * expected.
 * \return Returns delay until next communication session in seconds.
 * \param[in] hashLen Length of the string to write hash
 * \~Russian Извлекает задержку до следующего сеанса связи из стркои, полученной
 * от сервера ОРВД.
 * \param[in] response Строка, пришедшая от сервера ОРВД. Ожидается наличие
 * подстроки "$Delay задержка#".
