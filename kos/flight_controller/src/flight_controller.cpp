 ...
 * \note Considered end characters: "_" -- separates arguments," & "-- separates
 * commands and
 ...
 * \note Концом записи считаются: "_", отделяющий аргументы, "&" -- отделяющий
 * команды и
...
/**
 * \~English Recognizes and converts fractional number from mission string to an
 * integer.
 * \param [in, out] str Pointer to a string. Will be shifted to the beginning of
 * the next number/command.
 * \param [out] value Recognized number.
 * \param [in] numAfterPoint Number of decimal places. Excess places will be
 * discarded. Missing zeroes will be added.
 * \return Returns 1 on successful parse, 0 otherwise.
 * \~Russian Распознает и преобразует дробную запись одного числа в строке
 * миссии в целое.
 * \param[in, out] str Указатель на строку. Будет сдвинут к началу следующего
 * числа/команды.
 * \param[out] value Распознанное число.
 * \param[in] numAfterPoint Число точек, после запятой. Если в записи поданного
 * числа цифр после запятой больше указанного, лишние будут отброшены. Если в
 * записи поданного числа цифр после запятой меньше указанного, недостающие нули
 * будут добавлены.
...
int parseInt(char *&string, int32_t &value, uint32_t numAfterPoint) {
  ...
  while (string[strPtr] != '.')
    if (valPtr >= COMMAND_MAX_STRING_LEN) {
      logEntry("Failed to parse int value: too long", ENTITY_NAME,
      ...
      logEntry("Failed to parse int value: contains non-digit symbols",
  ...
  for (int j = i; j < numAfterPoint; j++)
    if (valPtr >= COMMAND_MAX_STRING_LEN) {
      logEntry(
          "Failed to parse int value: too many digits after point is required",
          ENTITY_NAME, LogLevel::LOG_WARNING);
      return 0;
    } else {
 ...
 * \param[in] str Pointer to mission string. Parses commands until "#"
 * character.
 * \return Returns 1 on successful parse, 0 otherwise.
...
int parseCommands(char *str) {
  ...
  for (uint32_t i = 0;; i++)
    if (str[i] == '#')
      break;
    else if (str[i] == 'H' || str[i] == 'T' || str[i] == 'W' || str[i] == 'L' ||
             str[i] == 'S' || str[i] == 'D' || str[i] == 'I')
      commandNum++;
    else if (str[i] == '\0') {
      logEntry("Cannot parse commands: no correct ending", ENTITY_NAME,
               LogLevel::LOG_WARNING);
      return 0;
    }
  if (commandNum == 0) {
    logEntry("Mission contains no commands", ENTITY_NAME,
             LogLevel::LOG_WARNING);
    return 0;
  }
  ...
  for (uint32_t i = 0; i < commandNum; i++) {
    ...
    for (uint32_t j = ptr + 1;; j++)
      if (str[j] == '&' || str[j] == '#') {
        ...
        logEntry("Failed to parse commands: unexpected string end", ENTITY_NAME,
                 LogLevel::LOG_WARNING);
        return 0;
      }
    ...
    switch (str[ptr]) {
    case 'H':
      if (!parseInt(stringPtr, lat, 7) || !parseInt(stringPtr, lng, 7) ||
          !parseInt(stringPtr, alt, 2)) {
        logEntry("Failed to parse values for 'home' command", ENTITY_NAME,
                 LogLevel::LOG_WARNING);
        free(commands);
        return 0;
      }
    ...
    case 'T':
      if (!parseInt(stringPtr, alt, 2)) {
        logEntry("Failed to parse values for 'takeoff' command", ENTITY_NAME,
                 LogLevel::LOG_WARNING);
        free(commands);
        return 0;
      }
    ...
    case 'W': {
      if (!parseInt(stringPtr, lat, 7) || !parseInt(stringPtr, lng, 7) ||
          !parseInt(stringPtr, alt, 2)) {
        logEntry("Failed to parse values for 'waypoint' command", ENTITY_NAME,
                 LogLevel::LOG_WARNING);
        free(commands);
        return 0;
      }
    ...
    case 'L':
      if (!parseInt(stringPtr, lat, 7) || !parseInt(stringPtr, lng, 7) ||
          !parseInt(stringPtr, alt, 2)) {
        logEntry("Failed to parse values for 'land' command", ENTITY_NAME,
                 LogLevel::LOG_WARNING);
        free(commands);
        return 0;
      }
    ...
    case 'S': {
      ...
      if (!parseInt(stringPtr, num, 0) || !parseInt(stringPtr, pwm, 0)) {
        logEntry("Failed to parse values for 'set servo' command", ENTITY_NAME,
                 LogLevel::LOG_WARNING);
        free(commands);
        return 0;
      }
    ...
    case 'D': {
      if (!parseInt(stringPtr, alt, 0)) {
        logEntry("Failed to parse values for 'delay' command", ENTITY_NAME,
                 LogLevel::LOG_WARNING);
        free(commands);
        return 0;
      }
    ...
    case 'I': {
      if (!parseInt(stringPtr, lat, 7) || !parseInt(stringPtr, lng, 7) ||
          !parseInt(stringPtr, alt, 2)) {
        logEntry("Failed to parse values for 'interest' command", ENTITY_NAME,
                 LogLevel::LOG_WARNING);
        free(commands);
        return 0;
      }
    ...
    default: {
      ...
      snprintf(logBuffer, 256, "Cannot parse an unknown command '%c'",
               str[ptr]);
 ...
 * \param[in] str Pointer to no-flight areas string. Parses areas until "#"
 * character.
 * \return Returns 1 on successful parse, 0 otherwise.
 * \~Russian Распознает бесполетные зоны.
 * \param[in] str Указатель на строку с бесполетными зонами. Распознает зоны до
 * символа "#".
...
int parseAreas(char *str) {
  ...
  if (!parseInt(str, num, 0)) {
    logEntry("Failed to parse a number of no-flight areas", ENTITY_NAME,
             LogLevel::LOG_WARNING);
    return 0;
  }
  areaNum = num;
  ...
  for (int i = 0; i < areaNum; i++) {
    ...
    if (nameEnd == NULL) {
      char logBuffer[256];
      snprintf(logBuffer, 256, "Failed to parse a name of no-flight area %d",
               i + 1);
      logEntry(logBuffer, ENTITY_NAME, LogLevel::LOG_WARNING);
      return 0;
    }
    ...
    if (!parseInt(str, pointNum, 0)) {
      ...
      snprintf(logBuffer, 256,
               "Failed to parse a number of no-flight area %d points", i + 1);
      logEntry(logBuffer, ENTITY_NAME, LogLevel::LOG_WARNING);
      return 0;
    }
    ...
    for (int j = 0; j < pointNum; j++)
      if (!parseInt(str, areas[i].points[j].latitude, 7) ||
          !parseInt(str, areas[i].points[j].longitude, 7)) {
        char logBuffer[256];
        snprintf(logBuffer, 256,
                 "Failed to parse point %d of no-flight area %d", j + 1, i + 1);
        logEntry(logBuffer, ENTITY_NAME, LogLevel::LOG_WARNING);
        return 0;
      }
  }
 ...
 * \param[in] str Pointer to no-flight areas changes string. Parses changes
 * until "#" character.
 * \return Returns 1 on successful parse, 0 otherwise.
 * \~Russian Распознает изменения бесполетных зон.
 * \param[in] str Указатель на строку с изменениями бесполетных зон. Распознает
 * изменения до символа "#".
 * \return Возвращает 1, если изменения были распознаны, иначе -- 0.
 */
int parseAreasDelta(char *str) {
  int32_t num;
  if (!parseInt(str, num, 0)) {
    logEntry("Failed to parse a number of no-flight areas changes", ENTITY_NAME,
             LogLevel::LOG_WARNING);
    return 0;
  }
  ...
  for (int i = 0; i < num; i++) {
    ...
    if (end == NULL) {
      char logBuffer[256];
      snprintf(logBuffer, 256,
               "Failed to parse delta type of no-flight area %d", i + 1);
      logEntry(logBuffer, ENTITY_NAME, LogLevel::LOG_WARNING);
      return 0;
    }
    len = end - str;
    ...
    if (!strcmp(modeType, "modified")) {
      int32_t pointNum;
      if (!parseInt(str, pointNum, 0)) {
        logEntry("Failed to parse point number of no-flight area to modify",
                 ENTITY_NAME, LogLevel::LOG_WARNING);
        return 0;
      }
      Point2D *points = (Point2D *)malloc(pointNum * sizeof(Point2D));
      for (int j = 0; j < pointNum; j++)
        if (!parseInt(str, points[j].latitude, 7) ||
            !parseInt(str, points[j].longitude, 7)) {
          char logBuffer[256];
          snprintf(logBuffer, 256,
                   "Failed to parse point %d of no-flight area to modify",
                   j + 1);
          logEntry(logBuffer, ENTITY_NAME, LogLevel::LOG_WARNING);
          return 0;
        }
      ...
      areas =
          (NoFlightArea *)realloc(areas, (areaNum + 1) * sizeof(NoFlightArea));
      strncpy(areas[areaNum].name, areaName, strlen(areaName));
      if (!parseInt(str, areas[areaNum].pointNum, 0)) {
        char logBuffer[256];
        snprintf(logBuffer, 256,
                 "Failed to parse a number of no-flight area %d points",
                 areaNum + 1);
        logEntry(logBuffer, ENTITY_NAME, LogLevel::LOG_WARNING);
        return 0;
      }
      areas[areaNum].points =
          (Point2D *)malloc(areas[areaNum].pointNum * sizeof(Point2D));
      for (int k = 0; k < areas[areaNum].pointNum; k++)
        if (!parseInt(str, areas[areaNum].points[k].latitude, 7) ||
            !parseInt(str, areas[areaNum].points[k].longitude, 7)) {
          char logBuffer[256];
          snprintf(logBuffer, 256,
                   "Failed to parse point %d of no-flight area %d", k + 1,
                   areaNum + 1);
          logEntry(logBuffer, ENTITY_NAME, LogLevel::LOG_WARNING);
          return 0;
        }
      ...
      if (!parseInt(str, pointNum, 0)) {
        logEntry("Failed to parse point number of no-flight area to delete",
                 ENTITY_NAME, LogLevel::LOG_WARNING);
        return 0;
      }
      for (int k = 0; k < pointNum; k++)
        if (!parseInt(str, coord, 7) || !parseInt(str, coord, 7)) {
          char logBuffer[256];
          snprintf(logBuffer, 256,
                   "Failed to parse point %d of no-flight area to delete",
                   k + 1);
          logEntry(logBuffer, ENTITY_NAME, LogLevel::LOG_WARNING);
          return 0;
        }
      for (int j = 0; j < areaNum; j++)
        if (!strcmp(areas[j].name, areaName)) {
          ...
          areaNum--;
          areas =
              (NoFlightArea *)realloc(areas, areaNum * sizeof(NoFlightArea));
          break;
        }
    } else {
      char logBuffer[256];
...
int loadMission(char *mission) {
  if (strstr(mission, "$-1#") != NULL) {
    logEntry("No mission is available on the server", ENTITY_NAME,
             LogLevel::LOG_WARNING);
    return 0;
  }
  ...
  if (start == NULL) {
    logEntry("Response from the server does not contain mission", ENTITY_NAME,
             LogLevel::LOG_WARNING);
    return 0;
  }
  start += strlen(header);
...
void printMission() {
  ...
  for (int i = 0; i < commandNum; i++) {
    ...
    case CommandType::HOME:
      snprintf(logBuffer, 256, "Home: %d, %d, %d",
    ...
    case CommandType::TAKEOFF:
      snprintf(logBuffer, 256, "Takeoff: %d",
               commands[i].content.takeoff.altitude);
      logEntry(logBuffer, ENTITY_NAME, LogLevel::LOG_INFO);
      break;
    case CommandType::WAYPOINT:
      snprintf(logBuffer, 256, "Waypoint: %d, %d, %d",
               commands[i].content.waypoint.latitude,
               commands[i].content.waypoint.longitude,
               commands[i].content.waypoint.altitude);
      logEntry(logBuffer, ENTITY_NAME, LogLevel::LOG_INFO);
    ...
    case CommandType::LAND:
      snprintf(logBuffer, 256, "Land: %d, %d, %d",
               commands[i].content.waypoint.latitude,
               commands[i].content.waypoint.longitude,
               commands[i].content.waypoint.altitude);
      logEntry(logBuffer, ENTITY_NAME, LogLevel::LOG_INFO);
    ...
    case CommandType::SET_SERVO:
      snprintf(logBuffer, 256, "Set servo: %d, %d",
               commands[i].content.servo.number, commands[i].content.servo.pwm);
      logEntry(logBuffer, ENTITY_NAME, LogLevel::LOG_INFO);
      break;
    ...
    case CommandType::INTEREST:
      snprintf(logBuffer, 256, "Point of interest: %d, %d, %d",
               commands[i].content.waypoint.latitude,
               commands[i].content.waypoint.longitude,
               commands[i].content.waypoint.altitude);
      logEntry(logBuffer, ENTITY_NAME, LogLevel::LOG_INFO);
...
int missionToString(MissionCommand *commands, uint8_t num, char *string,
                    uint32_t len) {
  for (int i = 0; i < num; i++) {
    int commandLen = 0;
    char command[256] = {0};
    ...
    case CommandType::SET_SERVO:
      commandLen =
          snprintf(command, 256, "S%d_%d", commands[i].content.servo.number,
                   commands[i].content.servo.pwm);
      break;
    case CommandType::DELAY:
      commandLen =
          snprintf(command, 256, "D%d", commands[i].content.delay.delay);
      break;
    case CommandType::INTEREST:
      coordToString(latStr, 13, commands[i].content.waypoint.latitude, 7);
      coordToString(lngStr, 13, commands[i].content.waypoint.longitude, 7);
...
int missionToBytes(MissionCommand *commands, uint8_t num, uint8_t *bytes) {
  ...
  for (int i = 0; i < num; i++) {
    ...
    case CommandType::LAND:
    case CommandType::INTEREST:
      memcpy(bytes + shift, &(commands[i].content.waypoint.latitude),
             sizeof(int32_t));
      shift += sizeof(int32_t);
      memcpy(bytes + shift, &(commands[i].content.waypoint.longitude),
             sizeof(int32_t));
      shift += sizeof(int32_t);
      memcpy(bytes + shift, &(commands[i].content.waypoint.altitude),
             sizeof(int32_t));
      shift += sizeof(int32_t);
      break;
    case CommandType::TAKEOFF:
      memcpy(bytes + shift, &(commands[i].content.takeoff.altitude),
             sizeof(int32_t));
      shift += sizeof(int32_t);
      break;
    case CommandType::SET_SERVO: {
...
int loadNoFlightAreas(char *areas) {
  ...
  if (start == NULL) {
    logEntry("Response from the server does not contain no-flight areas",
             ENTITY_NAME, LogLevel::LOG_WARNING);
    return 0;
  }
  start += strlen(header);
...
int updateNoFlightAreas(char *areas) {
  ...
  char *start = strstr(areas, header);
  if (start == NULL) {
    logEntry("Response from the server does not contain no-flight areas delta",
             ENTITY_NAME, LogLevel::LOG_WARNING);
    return 0;
  }
  start += strlen(header);
...
void printNoFlightAreas() {
  ...
  logEntry(logBuffer, ENTITY_NAME, LogLevel::LOG_INFO);
  for (int i = 0; i < areaNum; i++) {
    snprintf(logBuffer, 256, "Area '%s' contains %d points", areas[i].name,
             areas[i].pointNum);
    logEntry(logBuffer, ENTITY_NAME, LogLevel::LOG_INFO);
    for (int j = 0; j < areas[i].pointNum; j++) {
      snprintf(logBuffer, 256, "Point %d: %d, %d", j + 1,
               areas[i].points[j].latitude, areas[i].points[j].longitude);
      logEntry(logBuffer, ENTITY_NAME, LogLevel::LOG_INFO);
    }
  }
}
...
char *getNoFlightAreasHash() {
  if (!strlen(areasHash)) {
    ...
    for (int i = 0; i < areaNum; i++) {
      snprintf(areasString, 1024, "%s&%s&%d", areasString, areas[i].name,
               areas[i].pointNum);
      for (int j = 0; j < areas[i].pointNum; j++) {
        char lat[13] = {0};
        char lng[13] = {0};
    ...
    if (mbedtls_sha256_starts(&sha256, 0) != 0) {
      logEntry("Failed to calculate no-flight areas hash", ENTITY_NAME,
               LogLevel::LOG_WARNING);
      mbedtls_sha256_free(&sha256);
      return areasHash;
    }
    if (mbedtls_sha256_update(&sha256, (unsigned char *)areasString,
                              strlen(areasString)) != 0) {
      logEntry("Failed to calculate no-flight areas hash", ENTITY_NAME,
               LogLevel::LOG_WARNING);
      mbedtls_sha256_free(&sha256);
      return areasHash;
  ...
  if (char *begin = strstr(response, "$ForbiddenZonesHash ")) {
    ...
    if (char *end = strstr(begin, "$")) {
      ...
      if (len == 64)
        ...
        logEntry("Failed to parse hash: hash length is incorrect", ENTITY_NAME,
                 LogLevel::LOG_WARNING);
    } else
      logEntry("Failed to parse hash: end of hash is not found", ENTITY_NAME,
               LogLevel::LOG_WARNING);
  } else
    logEntry("Message does not contain hash", ENTITY_NAME,
             LogLevel::LOG_WARNING);
}
...
uint32_t parseDelay(char *response) {
  ...
  logEntry("Failed to parse delay until next session: setting it to 1s",
           ENTITY_NAME, LogLevel::LOG_WARNING);
  return 1;
}
...
int requestRecognition() {
  ...
  if (!publishMessage("api/image/request", publication.c_str())) {
    logEntry("Failed to send an image to AI server", ENTITY_NAME,
             LogLevel::LOG_WARNING);
    return 0;
  }
...
int getRecognitionResponse(char *tag, int32_t &alt) {
  ...
  if (!tagStart) {
    logEntry("Failed to parse AI server response", ENTITY_NAME,
             LogLevel::LOG_WARNING);
    return 0;
  }
