...
start-orvd: ## Запуск ОрВД в docker контейнере
	...
	[ -n "$$mqttuser" ] || read -p "Please enter MQTT username: " mqttuser; \
	[ -n "$$mqttpass" ] || read -p "Please enter MQTT password: " mqttpass; \
	docker run --name orvd -ti -e MQTT_HOST=$$mqttserver -e MQTT_USERNAME=$$mqttuser -e MQTT_PASSWORD=$$mqttpass -w /home/user/orvd -p 8080:8080 --rm orvd
